            ...
            if costmap_grid_[y][x] == 100 {
                ...
                for (int dy = -inflation_cells; dy <= inflation_cells; ++dy){ // Across and down 
                    for (int dx = -inflation_cells; dx <= inflation_cells; ++dx){
                        ...
                        // calculate pythag distance from the obstacle cell * resolution (convert to meters)
                        double dist = std::sqrt(dx*dx + dy*dy) * resolution
                        if (dist > inflation_radius) continue; // if dist > 10 blocks (meters) away skip 

                        // cost = 99 x (1 - dist / inflation_radius)
                        // only assign if higher than whats already there 
                        int cost = max_cost * (1 - dist / inflation_radius)
                        inflated[ny][nx] == std::max(inflated[ny][nx], cost) // only inflate that cell 
...
void CostmapCore::publishCostmap(){ 
    ...
    msg.info.origin.position.w = 1.0 // no rotation 
